...
# A, paths
train = '/Users/Hubert/Development/机器学习大作业/Data/train.csv'               # path to training file
test = '/Users/Hubert/Development/机器学习大作业/Data/test.csv'                 # path to testing file
...
submission = '/Users/Hubert/Development/机器学习大作业/Data/submission26-{0}-{1}-{2}-{3}.csv'.format(alpha,beta,L1,L2)  # path of to be outputted submission file
...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        # for field in ['C1','C14','C15','C16','C17','C18','C19','C20','C21']:
        #     del row[field]

        if row['app_category'] == '07d7df22':
            row['p'] = 'site'
            row['p_id'] = row['site_id']
            row['p_domain'] = row['site_domain']
            row['p_category'] = row['site_category']
        else:
            row['p'] = 'app'
            row['p_id'] = row['app_id']
            row['p_domain'] = row['app_domain']
            row['p_category'] = row['app_category']
        for field in ['site_id','site_domain','site_category','app_id','app_domain','app_category', 'device_ip']:
            del row[field]
